...
import {HTML_CLASS} from "../scripts/src/html-properties.js";
...
function checkMarkingOnCalendar() {
    ...
    expect($(document).find(HTML_TAG_NAME.crossSign).length).to.eql(1);
}
...
function checkUnmarkingOnCalendar() {
    ...
    expect($(document).find(HTML_TAG_NAME.crossSign).length).to.eql(0);
}